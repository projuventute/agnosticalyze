...
try {
	...
	intervalLoop = setInterval(function() {
			if (typeof window.TMSHelper === "object" && typeof window.TMSProcessing === "object") { // TMS is ready
						...
						if (that.matches && that.matches(relevantElements)) {
							if (typeof that === "object") {
								...
								if (that.hasAttribute("data-track-prevent")) {
									window.TMSHelper.console("[MERCI] -> info: tracking prevention flag found on element, TMS Event Tracking not triggered");
								} else if (typeof window.TMSProcessing === "object" && typeof window.TMSProcessing.runEventCollectionOrchestrator === "function") {
									window.TMSHelper.console("[MERCI] -> info: triggering TMS Event Tracking");
									event_data = {};
									event_data["event_eventInfo_type"] = "user-interaction";
									event_data["event_processing_trigger"] = "merci";
									event_data = window.TMSProcessing.runEventCollectionOrchestrator(that, event_data);
									...
									event_data = window.TMSHelper.copyVarsFromObjectToObject({
										sourceObject: window.TMSProcessing.identifyStandardEventCase(event_data),
									...
									if (typeof window.TMSProcessing.setEventTriggerFlag === "function") {
										event_data = window.TMSProcessing.setEventTriggerFlag(event_data);
									}
									...
									if (typeof event_data["event_processing_sendRequest"] === "boolean" && event_data["event_processing_sendRequest"]) {
										window.TMSHelper.console("[MERCI] -> info: tracking conditionally approved, DICE triggered");
										window.TMSProcessing.dice(event_data);
									} else {
										window.TMSHelper.console("[MERCI] -> info: tracking conditionally prevented, DICE not triggered");
									}
								} else {
									window.TMSHelper.console("[MERCI] -> warning: TMSProcessing.runEventCollectionOrchestrator not found");
								}
						
								window.TMSHelper.console("[MERCI] complete");
							} else {
								window.TMSHelper.console("[MERCI] -> warning: No valid interacted element 'that' identified");
							} // end: if 'that' exists
						}
					}
				});
			...
			clearInterval(intervalLoop);
			window.console.log("[MERCI setup] -> warning: waited too long, MERCI not set up");
		} else {
			window.console.log("[MERCI setup] -> info: TMS not ready, trying again in 0.5 seconds...");
			intervalCounter++;
		}
	}, 500);
} catch (err) {
	window.console.log("[MERCI setup] error:");
	...
	intervalLoop = setInterval(function() {
			if (typeof window.TMSHelper === "object" && typeof window.TMSProcessing === "object") { // TMS is ready
				...
				window.TMSHelper.console("[ReceiveFromChildWindow setup] complete");
					...
					clearInterval(intervalLoop);
					window.console.log("[ReceiveFromChildWindow setup] -> warning: waited too long, ReceiveFromChildWindow not set up");
			} else {
				window.console.log("[ReceiveFromChildWindow setup] -> info: TMS not ready, trying again in 0.5 seconds...");
					intervalCounter++;
			}
	}, 500);
	} catch (err) {
		window.console.log("[ReceiveFromChildWindow setup] error:");
		window.console.error(err);
	};